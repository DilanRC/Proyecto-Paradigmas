...
try {
    ...
    Invoke-DockerProcess ($BaseArgs + 'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" --single-transaction --routines --triggers --events --no-tablespaces --set-gtid-purged=OFF --default-character-set=utf8mb4 dbtindercows') $CompleteFile
    Invoke-DockerProcess ($BaseArgs + 'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" --no-data --routines --triggers --events --no-tablespaces --set-gtid-purged=OFF --default-character-set=utf8mb4 dbtindercows') $SchemaFile
    Invoke-DockerProcess ($BaseArgs + 'exec mysqldump -uroot -p"$MYSQL_ROOT_PASSWORD" --no-create-info --single-transaction --skip-triggers --no-tablespaces --set-gtid-purged=OFF --default-character-set=utf8mb4 dbtindercows') $DataFile
