...
SET
    ...
    tbproductorEstado = 1
WHERE
    tbproductorIdentificacionNumero = '101110111';
...
SET
    ...
    tbproductorEstado = 1
WHERE
    tbproductorIdentificacionNumero = '3101111111';
...
SELECT 1, '101110111', 'CEDULA_FISICA', 'Maria Fernandez Solano', '88881111', 'contacto.compartido@example.test', 1
WHERE
    NOT EXISTS (
        SELECT 1
        FROM tbproductor
...
UNION ALL
SELECT 2, '3101111111', 'CEDULA_JURIDICA', 'Ganaderia Valle Verde S.A.', '+50622221111', 'contacto.compartido@example.test', 1
WHERE
    NOT EXISTS (
        SELECT 1
        FROM tbproductor
...
-- === BLOQUE CORREGIDO: se agrega tbproductordireccionId explícito ===
...
UPDATE tbproductorfinca
SET
    tbproductorfincaEstado = 1
WHERE (
        tbproductorId = 1
        AND tbproductorfincaNombre IN (
            'Finca El Roble',
            'Finca Valle Verde'
        )
    )
    OR (
        tbproductorId = 2
        AND tbproductorfincaNombre = 'Finca Valle Verde'
    );
...
WHERE
    NOT EXISTS (
        ...
        WHERE
            tbproductorId = 1
            AND tbproductorfincaNombre = 'Finca El Roble'
    )
UNION ALL
