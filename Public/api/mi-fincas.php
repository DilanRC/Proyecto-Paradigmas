...
foreach (['NamedLock', 'Persona', 'ProductorFinca', 'Direccion', 'FincaDireccion', 'Bitacora', 'Productor'] as $modelo) require_once $raiz . "/Application/Model/{$modelo}.php";
require_once $raiz . '/Application/Controller/MiFincasController.php';

$metodo = strtoupper($_SERVER['REQUEST_METHOD'] ?? 'GET');
$permitidos = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'];
if ($metodo === 'OPTIONS') { header('Allow: GET, POST, PUT, DELETE, PATCH, OPTIONS'); http_response_code(204); exit; }
if (!in_array($metodo, $permitidos, true)) { header('Allow: GET, POST, PUT, DELETE, PATCH, OPTIONS'); sendJsonResponse(['success' => false, 'message' => 'Método no permitido.', 'data' => null], 405); }
$conCuerpo = $metodo !== 'GET';
if ($conCuerpo && strtolower(trim(explode(';', $_SERVER['CONTENT_TYPE'] ?? '')[0])) !== 'application/json') sendJsonResponse(['success' => false, 'message' => 'El cuerpo debe usar Content-Type: application/json.', 'data' => null], 415);
try {
    ...
    $respuesta = (new MiFincasController($conexion, $actor, is_string($_SERVER['HTTP_X_REQUEST_ID'] ?? null) ? $_SERVER['HTTP_X_REQUEST_ID'] : null))->procesar($metodo, $cuerpo);
    ...
    sendJsonResponse(['success' => false, 'message' => $excepcion->getMessage(), 'data' => $excepcion->datos, 'errors' => $excepcion->errores], $excepcion->estadoHttp);
